...
//    private AccessibilityScrollData lastScrollData;
    ...
    public static boolean shouldUseCompactResponses() {
        ...
        if (Session.capabilities.containsKey(Settings.SHOULD_USE_COMPACT_RESPONSES.toString())) {

            shouldUseCompactResponses = Boolean.valueOf((Session.capabilities.get(Settings.SHOULD_USE_COMPACT_RESPONSES.toString()).toString()));
        }
        return shouldUseCompactResponses;
    }

    public static String[] getElementResponseAttributes() {
        if (Session.capabilities.containsKey(Settings.ELEMENT_RESPONSE_ATTRIBUTES.toString())) {
            return Session.capabilities.get(Settings.ELEMENT_RESPONSE_ATTRIBUTES.toString()).toString()
                    .split(",");
        }
