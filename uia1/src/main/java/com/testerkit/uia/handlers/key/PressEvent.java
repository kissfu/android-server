...
public abstract class PressEvent extends SafeRequestHandler {
    public PressEvent(String mappedUri) {
        super(mappedUri);
    }
    ...
    protected AppiumResponse safeHandle(IRequest request) throws Exception {
        ...
        if(step == null || step.getKey() == null || executePressEvent() == false){
            return new AppiumResponse(getSessionId(request), WDStatus.UNKNOWN_ERROR, String.format(
                    "Cannot generate key press event for key event %s", step));
        }
